...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about a Moonbeam node"
		echo
...
main() {
	...
	# Send Pull request with new texts to add a language - https://github.com/SecorD0/Moonbeam/blob/main/node_info.sh
	#elif [ "$language" = ".." ]; then
	else
		local t_nn="\nMoniker:                  ${C_LGn}%s${RES}"
	...
	if [ "$r_port" -eq "$p_port" ] || [ ! -n "$r_port" ]; then
		r_port=9934
	fi
